...
$text_align  = o100ne_get_text_align();
...
$show_image         = isset( $element_data['show_image'] ) ? filter_var($element_data['show_image'], FILTER_VALIDATE_BOOLEAN) : (isset( $o100ne_settings['show_product_image'] ) ? boolval( $o100ne_settings['show_product_image'] ) : false);
$show_sku           = isset( $element_data['show_sku'] ) ? filter_var($element_data['show_sku'], FILTER_VALIDATE_BOOLEAN) : (isset( $o100ne_settings['show_product_sku'] ) ? boolval( $o100ne_settings['show_product_sku'] ) : false);
$show_price         = isset( $element_data['show_price'] ) ? filter_var($element_data['show_price'], FILTER_VALIDATE_BOOLEAN) : true;
...
foreach ( $order_items as $item_id => $item ) :
    ...
    <tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order_data ) ); ?>" style="<?php echo esc_attr( $text_style ); ?>;">
        ...
        foreach ( $structure_items as $key => $structure_item ) :
            ...
            ?>
            <td colspan="<?php echo esc_attr( $structure_item['col_span'] ); ?>" class="td o100ne_item_<?php echo esc_attr( $key ); ?>_content" style="<?php echo esc_attr( $column_style ); ?>">
            <?php
            switch ( $key ) :
                case 'product':
                    ...
                    ?>
                    <div class="o100ne-wc-item-meta" style="clear:both">
                    <?php
                        wc_display_item_meta(
                            $item,
                            [
                                'label_before' => '<strong class="wc-item-meta-label o100ne-item-meta-label" style="float: ' . esc_attr( $text_align ) . '; margin-' . esc_attr( $margin_side ) . ': .25em; clear: both">',
                            ]
                        );
                    ?>
                    </div>
                ...
                case 'price':
                    ...
                    if ( ( $show_regular_price && ! empty( $product_regular_price ) ) || ( $is_placeholder && ! empty( $product_regular_price ) ) ) {
                        require O100NE_PLUGIN_PATH . 'templates/shortcodes/order-details/order-items/product-regular-price-content.php';
                    }
                        echo wp_kses_post( $order_data->get_formatted_line_subtotal( $item ) );
                    break;
                default:
                    echo wp_kses_post( do_action( 'o100_order_details_item_' . $key . '_content', $item, $order_data, $element_data, false ) );
                    break;
            endswitch;
            ?>
            </td>
    ...
    if ( $show_purchase_note && $purchase_note ) {
        ?>
        <tr>
            <td class="o100ne-purchase-note" colspan="<?php echo isset( $structure_items['cost'] ) ? 3 : 2; ?>" style="<?php echo esc_attr( $text_style ); ?>;">
                <?php
