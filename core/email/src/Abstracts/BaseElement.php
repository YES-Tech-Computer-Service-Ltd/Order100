...
abstract class BaseElement {
    ...
    public static function get_layout( $element_data, $args ) {
        ...
        return apply_filters( 'o100_' . static::$type . '_layout', o100ne_get_content( $path, array_merge( [ 'element' => $element_data ], $args ) ), $element_data, $args );
    }
    ...
    public function is_available_in_email( $email ) {
        ...
        $available_email_ids = apply_filters( 'o100_element_available_email_ids', $available_email_ids, $this );

        return ( in_array( O100NE_ALL_EMAILS, $available_email_ids, true ) )
        || ( in_array( O100NE_NON_ORDER_EMAILS, $available_email_ids, true ) && in_array( O100NE_NON_ORDER_EMAILS, $email->email_types, true ) )
        || ( in_array( O100NE_WITH_ORDER_EMAILS, $available_email_ids, true ) && in_array( O100NE_WITH_ORDER_EMAILS, $email->email_types, true ) )
        || ( in_array( O100NE_GLOBAL_HEADER_FOOTER_ID, $available_email_ids, true ) && in_array( O100NE_GLOBAL_HEADER_FOOTER_ID, $email->email_types, true ) )
