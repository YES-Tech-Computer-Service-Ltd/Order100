...
class OrderMetaShortcodes {
    ...
    protected function __construct() {
        ...
        add_filter( 'o100_extra_shortcodes', [ $this, 'get_order_meta_shortcodes' ], 10, 2 );
        add_filter( 'o100_extra_shortcodes', [ $this, 'get_order_tax_shortcodes' ], 10, 2 );
    }
    ...
    public function get_order_tax_shortcodes( $shortcodes, $data ) {
        ...
        foreach ( $tax_items as $item_id => $item_tax ) {
            ...
            $new_shortcode = [
                'name'          => "o100_order_taxes_{$tax_rate_id}",
    ...
    public function order_meta_callback( $data, $shortcode_attrs = [] ) {
        ...
        if ( is_array( $value ) || is_object( $value ) ) {
            ...
            ? $value['extra']
                : $this->o100ne_flatten_to_string( $value );
    ...
    protected function o100ne_flatten_to_string( $value ) {
        if ( is_array( $value ) || is_object( $value ) ) {
            $result = [];
            foreach ( (array) $value as $v ) {
                $result[] = $this->o100ne_flatten_to_string( $v );
            }
