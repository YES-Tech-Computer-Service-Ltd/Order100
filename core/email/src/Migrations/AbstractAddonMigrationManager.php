...
abstract class AbstractAddonMigrationManager {
    ...
    protected function init_hooks() {
        add_action( 'o100_run_addon_migrations', [ $this, 'execute_migrations' ] );
        add_filter( 'o100_required_migration_names', [ $this, 'check_if_addon_migration_needed' ] );
        add_filter( 'o100_migration_backup_data', [ $this, 'add_additional_backup_data' ] );
    }
    ...
    public function execute_migrations() {
        ...
        if ( empty( $this->addon_migrations ) ) {
            ...
            update_option( 'o100_addon_versions', $addon_versions );
        }
    ...
    public function check_if_addon_migration_needed( $needed_migrations ) {
        ...
        $successful_migrations = get_option( '_o100ne_successful_migrations', [] );
    ...
    public function add_additional_backup_data( $backup_data ) {
        ...
        $o100ne_addon_versions_option = null;

        foreach ( $options as &$option ) {
            if ( $option->option_name === 'o100_addon_versions' ) {
                $o100ne_addon_versions_option = &$option;
                break;
            }
        }

        // If the option is found
        if ( $o100ne_addon_versions_option ) {
            $versions = maybe_unserialize( $o100ne_addon_versions_option->option_value );
            ...
            if ( is_array( $versions ) ) {
                ...
                $o100ne_addon_versions_option->option_value = maybe_serialize( $versions );
            } else {
                throw new \Error( esc_html( "Error unserializing {$this->addon_namespace} version data" ), 500 );
            }
        } else {
            // If 'o100_addon_versions' option does not exist, create it
            $options[] = (object) [
                'option_name'  => 'o100_addon_versions',
    ...
    public function is_v4_supported() {
        foreach ( $this->supported_template_ids as $template_id ) {
            $args = [
                ...
                'meta_key'    => O100neTemplate::META_KEYS['name'],
            ...
            foreach ( $post_ids as $post_id ) {
                $is_v4_supported = get_post_meta( $post_id, O100neTemplate::META_KEYS['is_v4_supported'], true );
                if ( ! $is_v4_supported ) {
                    return false;
                }
            }
