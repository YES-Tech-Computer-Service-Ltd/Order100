...
/**
 * Script to migrate from O100ne legacy (pre 4.0.0) to 4.0.0
 */
final class Ver_4_0_0 extends AbstractMigration {
    ...
    private function migrate_templates() {
        ...
        foreach ( $template_posts as $template ) {
            ...
            $elements = get_post_meta( $this->current_template_id, \Order100\Notification\Engine\O100neTemplate::META_KEYS['elements'], true );
            ...
            update_post_meta( $this->current_template_id, \Order100\Notification\Engine\O100neTemplate::META_KEYS['elements'], $elements );
            /**
             * ==========================
             */
             ...
             */
            // Template status
            $old_activation_status = get_post_meta( $this->current_template_id, \Order100\Notification\Engine\O100neTemplate::META_KEYS['status'], true );
            ...
            update_post_meta( $this->current_template_id, \Order100\Notification\Engine\O100neTemplate::META_KEYS['status'], $new_activation_status );
            ...
            update_post_meta( $this->current_template_id, \Order100\Notification\Engine\O100neTemplate::META_KEYS['background_color'], $old_outer_bg_color );
    ...
    private function migrate_o100ne_settings() {
        $this->logger->log( 'Start migrating O100ne settings' );
        ...
        foreach ( $attributes_map as $old_key => $new_key ) {
            $value = Helpers::get_object_value( $o100ne_settings, $old_key );
            ...
            Helpers::set_object_value( $o100ne_settings, $new_key, $value );
        }//end foreach
        ...
        $o100ne_settings[ SettingModel::META_KEYS['payment_display_mode'] ] = $payment_display_mode_map[ $o100ne_settings[ SettingModel::META_KEYS['payment_display_mode'] ] ] ?? 'yes';

        $o100ne_settings[ SettingModel::META_KEYS['product_image_position'] ] = strtolower( $o100ne_settings[ SettingModel::META_KEYS['product_image_position'] ] ?? 'top' );
        $o100ne_settings[ SettingModel::META_KEYS['product_image_width'] ]    = (int) str_replace( 'px', '', $o100ne_settings[ SettingModel::META_KEYS['product_image_width'] ] ?? '30' );
        $o100ne_settings[ SettingModel::META_KEYS['product_image_height'] ]   = (int) str_replace( 'px', '', $o100ne_settings[ SettingModel::META_KEYS['product_image_height'] ] ?? '30' );
        $o100ne_settings[ SettingModel::META_KEYS['container_width'] ]        = (int) str_replace( 'px', '', $o100ne_settings[ SettingModel::META_KEYS['container_width'] ] ?? '605' );
        if ( empty( $o100ne_settings[ SettingModel::META_KEYS['container_width'] ] ) ) {
            $o100ne_settings[ SettingModel::META_KEYS['container_width'] ] = 605;
        }
        /**
        ...
        // Update o100ne settings to db
        $update_o100ne_setting_options_result = update_option( SettingModel::OPTION_NAME, $o100ne_settings );
        if ( ! $update_o100ne_setting_options_result ) {
            $this->logger->log( 'Failed to update new O100ne settings to db' );
        }
        $this->logger->log( 'Done migrating O100ne settings' );
    }
    ...
    private function convert_element_data_attribute_values( &$element ) {
        ...
        if ( $element['type'] === 'logo' ) {
            ...
            if ( empty( $element['data']['src'] ) ) {
                $element['data']['src'] = O100NE_PLUGIN_URL . 'assets/images/woocommerce-logo.png';
    ...
    private function add_new_attributes_to_element_data( &$element ) {
        ...
        if ( $element['type'] === 'order_details' ) {
            ...
            $data['payment_instructions'] = '[o100_payment_instructions]';

            // Headers of the table columns/rows
            $order_details_titles         = get_post_meta( $this->current_template_id, '_o100ne_email_order_item_title', true );
            $data['title']                = $order_details_titles['order_title'] ?? '<span style="font-size: 20px;">Order #[o100_order_number] <b>([o100_order_date])</b></span>';
            $data['product_title']        = $order_details_titles['product_title'] ?? __( 'Product', 'order100' );
            $data['cost_title']           = $order_details_titles['cost_title'] ?? __( 'Cost', 'order100' );
            $data['quantity_title']       = $order_details_titles['quantity_title'] ?? __( 'Quantity', 'order100' );
            $data['price_title']          = $order_details_titles['price_title'] ?? __( 'Price', 'order100' );
            $data['cart_subtotal_title']  = $order_details_titles['subtoltal_title'] ?? __( 'Subtotal:', 'order100' );
            $data['payment_method_title'] = $order_details_titles['payment_method_title'] ?? __( 'Payment method:', 'order100' );
            $data['order_total_title']    = $order_details_titles['total_title'] ?? __( 'Total:', 'order100' );
            $data['order_note_title']     = $order_details_titles['customer_note'] ?? __( 'Note:', 'order100' );
            $data['shipping_title']       = $order_details_titles['shipping_title'] ?? __( 'Shipping:', 'order100' );
            $data['discount_title']       = $order_details_titles['discount_title'] ?? __( 'Discount:', 'order100' );
            return;
        }

        if ( $element['type'] === 'billing_address' ) {
            $data['rich_text'] = '[o100_billing_address]';
            return;
        }

        if ( $element['type'] === 'shipping_address' ) {
            $data['rich_text'] = '[o100_shipping_address]';
            return;
        }
        ...
        if ( $element['type'] === 'billing_shipping_address' ) {
            ...
            $data['shipping_address_content'] = '[o100_shipping_address]';
            $data['billing_address_content']  = '[o100_billing_address]';
            return;
        }

        if ( $element['type'] === 'order_details_download' ) {
            $data['title']          = __( 'Downloads', 'order100' );
            $data['product_title']  = __( 'Products', 'order100' );
            $data['expires_title']  = __( 'Expires', 'order100' );
            $data['download_title'] = __( 'Download', 'order100' );
            return;
        }
    ...
    private function migrate_shortcodes( &$data, $element_type = '' ) {
        $shortcodes_map = [
            '[o100_items_border_content]'         => '[o100_order_details]',
            '[o100_items_border_title]'           => '[Order #[o100_order_number]] ([o100_order_date])',
            '[o100_items_downloadable_product]'   => '[o100_order_details_download_product]',
            '[o100_user_account_url_string]'      => '[o100_user_account_url]',
            '[o100_quantity_count]'               => '[o100_order_product_item_count]',
            '[o100_orders_count]'                 => '[o100_order_product_line_item_count]',
            '[o100_orders_count_double]'          => '[o100_order_product_line_item_count_double]',
            '[o100_order_total_numbers]'          => '[o100_order_total_value]',
            '[o100_order_sub_total]'              => '[o100_order_subtotal]',
            '[o100_order_shipping]'               => '[o100_shipping_total]',
            '[o100_payment_instruction]'          => '[o100_payment_instructions]',
            '[o100_payment_method]'               => '[o100_order_payment_method]',
            '[o100_transaction_id]'               => '[o100_payment_transaction_id]',
            '[o100_set_password_url_string]'      => '[o100_set_password_url]',
            '[o100_order_payment_url_string]'     => '[o100_order_payment_url]',

            // Turn the shortcode into a plain hook name
            '[woocommerce_email_before_order_table]' => '[o100_custom_hook hook="woocommerce_email_before_order_table"]',
            '[woocommerce_email_after_order_table]'  => '[o100_custom_hook hook="woocommerce_email_after_order_table"]',
        ];

        if ( $element_type === 'heading' || $element_type === 'text' ) {
            $shortcodes_map['[o100_order_id]']     = '[o100_order_id is_plain="true"]';
            $shortcodes_map['[o100_order_number]'] = '[o100_order_number is_plain="true"]';
        }
