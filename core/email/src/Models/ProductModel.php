...
class ProductModel {
    ...
    /**
     * Maps a WC_Product object into the standard response array used by
     * FeaturedProducts and CrossUpSellsProducts email elements.
     *
     * @param \WC_Product|null $product A WooCommerce product object.
     * @return array|null Product data array or null if product is invalid.
     */
    public function get_product_response( $product ) {
        ...
        // Build formatted price HTML strings
        $sale_price_html    = ! empty( $sale_price ) ? wc_price( $sale_price ) : wc_price( $price );
        $regular_price_html = ! empty( $regular_price ) ? wc_price( $regular_price ) : '';

        // Use shop page URL with add-to-cart param for exfood popup flow
        $shop_url     = wc_get_page_permalink( 'shop' );
        $add_to_cart  = add_query_arg( 'add-to-cart', $product->get_id(), $shop_url );
