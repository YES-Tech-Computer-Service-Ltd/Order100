...
use Order100\Notification\Engine\Elements\Heading;
use Order100\Notification\Engine\Elements\Logo;
use Order100\Notification\Engine\Elements\Text;
use Order100\Notification\Engine\Elements\Footer;
...
class TemplateModel {
    ...
    private static $meta_keys     = O100neTemplate::META_KEYS;
    ...
    private static function query_template( $args = [] ) {
        ...
        $status_key = self::$meta_keys['status'];

        $clauses = [
            'select' => "SELECT posts.ID FROM $posts AS posts",
            'join'   => "JOIN $post_meta AS postmeta ON ( posts.ID = postmeta.post_id )"
                      . " LEFT JOIN $post_meta AS status_meta ON ( posts.ID = status_meta.post_id AND status_meta.meta_key = '$status_key' )",
            'where'  => "WHERE posts.post_type = '$post_type' AND posts.post_status IN ('publish', 'pending', 'future')",
            'order'  => "ORDER BY CASE WHEN status_meta.meta_value = 'active' THEN 0 ELSE 1 END, posts.ID DESC",
        ];
    ...
    /**
     * Finds all O100ne templates and retrieves relevant information about each template.
     *
     * @return array An array of O100ne templates with their corresponding data.
     ...
     *   - '...': Other template-specific data retrieved from O100ne.
     */
    public static function find_all() {
        ...
        // Also include templates that exist in DB but have no registered Email class
        // This covers templates created via the admin (e.g. loyalty, custom) that
        // don't appear in WC_Emails or o100ne_get_emails()
        ...
        foreach ( $db_templates as $post_id ) {
            $tpl_name = get_post_meta( $post_id, '_o100ne_template', true );
    ...
    /**
     * Get template data for template that is currently not editable by O100ne and O100ne addons
     * (It might or might not be supported by O100ne)
     *
    ...
    private static function get_uneditable_template( $wc_email, $templates ): array {
        ...
        $support_status_labels = [
            'pro_needed'    => __( 'Pro', 'order100' ),
            'addon_needed'  => __( 'Addon', 'order100' ),
            'not_supported' => __( 'N/A', 'order100' ),
        ];
    ...
    public static function insert( $args = false ) {
        ...
        update_post_meta( $new_template_id, self::$meta_keys['background_color'], O100NE_COLOR_BACKGROUND_DEFAULT );
        update_post_meta( $new_template_id, self::$meta_keys['is_v4_supported'], true );
        if ( ! empty( $args['text_link_color'] ) ) {
            update_post_meta( $new_template_id, self::$meta_keys['text_link_color'], O100NE_COLOR_WC_DEFAULT );
        }
    ...
    public static function update( $template_id, $data, $is_save_revision = false ) {
        if ( isset( $data['elements'] ) && isset( self::$meta_keys['elements'] ) ) {
            if ( is_array( $data['elements'] ) ) {
                // Legacy array-based elements — sanitize recursively
            ...
            } elseif ( is_string( $data['elements'] ) && ! empty( trim( $data['elements'] ) ) ) {
                // MJML string elements — store directly
                update_post_meta( $template_id, self::$meta_keys['elements'], $data['elements'] );
            }
        }

        // Persist template_elements_type (e.g. 'mjml') if provided
        if ( ! empty( $data['template_elements_type'] ) ) {
            update_post_meta( $template_id, '_o100ne_template_elements_type', sanitize_text_field( $data['template_elements_type'] ) );
        }

        if ( isset( $data['template_html'] ) ) {
            update_post_meta( $template_id, '_o100ne_template_html', $data['template_html'] );
        }
    ...
    public static function get_global_header_and_footer() {
        $query_args  = [
            'name' => 'o100_global_header_footer',
        ];
        ...
        if ( empty( $template_id ) ) {
            self::insert(
                [
                    'name'     => 'o100_global_header_footer',
                    'elements' => o100ne_get_default_elements( 'o100_global_header_footer' ),
                ]
            );
            $template_id = self::query_template( $query_args );
        }
        ...
        if ( empty( $elements ) ) {
             ...
             */
            $global_header_footer_instance = \Order100\Notification\Engine\Emails\GlobalHeaderFooter::get_instance();
    ...
    private static function get_meta_data( $template_post_id, $template_name = null ) {
        ...
        if ( $support_info['status'] !== 'already_supported' ) {
            ...
            $background_color         = self::query_meta_data( $template_post_id, self::$meta_keys['background_color'], O100neTemplate::DEFAULT_DATA['background_color'] );
            $text_link_color          = self::query_meta_data( $template_post_id, self::$meta_keys['text_link_color'], O100neTemplate::DEFAULT_DATA['text_link_color'] );
            $title_color              = self::query_meta_data( $template_post_id, self::$meta_keys['title_color'], O100neTemplate::DEFAULT_DATA['title_color'] );
            $content_background_color = self::query_meta_data( $template_post_id, self::$meta_keys['content_background_color'], O100neTemplate::DEFAULT_DATA['content_background_color'] );
        ...
        $filtered_elements = is_string( $template_elements )
            ? $template_elements
            : ElementsHelper::filter_available_elements( $template_elements, $template_name );
        ...
        return [
            ...
            'background_color'         => $background_color ?? O100NE_COLOR_BACKGROUND_DEFAULT,
            'text_link_color'          => $text_link_color ?? O100NE_COLOR_WC_DEFAULT,
    ...
    private static function get_uneditable_template_placeholder_elements( array $support_info ): array {
        ...
        $elements[] = BaseElement::reduce_new_element( Footer::get_data( [ 'rich_text' => '<p style="font-size: 14px;margin: 0px 0px 16px; text-align: center;">' . esc_html( get_bloginfo( 'name' ) ) . '&nbsp;- Built with <a style="color: ' . esc_attr( O100NE_COLOR_WC_DEFAULT ) . '; font-weight: normal; text-decoration: underline;" href="https://woocommerce.com" target="_blank" rel="noopener">WooCommerce</a></p>' ] ) );
    ...
    public static function get_shortcodes_by_template_name_and_order_id( $template_name, $order_id ) {
        do_action( 'o100_before_order_id_changed', $order_id );

        $shortcodes = o100ne_get_email_shortcodes( $template_name );

        // TODO: check shortcodes
        // $shortcodes = apply_filters( 'o100_extra_shortcodes', $shortcodes, $template_name, $order_id );
    ...
    /**
     * Gets data needed for executing a O100ne shortcode.
     *
    ...
    public static function get_elements_for_template( $template_id ): array {
        ...
        if ( $support_info['status'] !== 'already_supported' ) {
            $elements = array_map(
                ...
                o100ne_get_email_elements_data( 'new_order' )
            );
            return $elements;
        }
        return o100ne_get_email_elements_data( $template_id );
    }
