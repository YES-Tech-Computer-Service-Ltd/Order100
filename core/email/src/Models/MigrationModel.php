...
class MigrationModel {
    ...
    public function migrate() {
        ...
        if ( ! $is_migration_succeeded ) {
            throw new \Exception( esc_html__( 'Migration failed, please contact our Customer Support for help!', 'order100' ), 500 );
        }
    ...
    public function reset( $backup ) {
        ...
        try {
            ...
            foreach ( $backed_up_posts as $post ) {
                if ( ! isset( $post->post_type ) || 'o100_template' !== $post->post_type ) {
                    continue;
                }
                $wpdb->insert(
                    $wpdb->posts,
                ...
                if ( ! empty( $postmetas_for_current_post ) ) {
                    foreach ( $postmetas_for_current_post as $postmeta ) {
                        if ( ! isset( $postmeta->meta_key ) || strpos( (string) $postmeta->meta_key, 'order100' ) === false ) {
                            continue;
                        }
                        add_post_meta(
                            $inserted_post_id,
            ...
            foreach ( $backup['options'] as $option ) {
                // Only restore options that belong to O100ne (same pattern as export)
                if ( strpos( $option->option_name, 'order100' ) !== false ) {
            ...
            update_option( 'o100_version', $version );
            wp_cache_delete( 'o100_version', 'options' );

            // Restore addon versions
            $addon_versions          = get_option( 'o100_addon_versions', [] );
            ...
            update_option( 'o100_addon_versions', $restored_addon_versions );

            do_action( 'o100_before_reset_migration_commit', $backup, $removed_migrations );
            ...
            throw new \Exception( esc_html__( 'Restoration failed', 'order100' ), 500 );
        }//end try
    }
    ...
    private function get_required_migration_names() {
        $old_version           = MigrationHelper::format_version_number( get_option( 'o100_version' ) );
        $new_version           = MigrationHelper::format_version_number( O100NE_VERSION );
        ...
        if ( ( version_compare( $old_version, $new_version, '>=' ) && empty( $successful_migrations ) ) ) {
            return apply_filters( 'o100_required_migration_names', [] );
        }
        ...
        if ( ( $old_version == null && ! $has_o100ne_template ) && empty( $successful_migrations ) ) {
            return [];
        }

        // Check if any migrations have already been run up to current version
        foreach ( $successful_migrations as $migration ) {
            if ( empty( $migration['addon_namespace'] ) && version_compare( $migration['to_version'], O100NE_VERSION, '>=' ) ) {
                return apply_filters( 'o100_required_migration_names', [] );
            }
        }
        ...
        $needed_migrations = empty( $filtered_migrations ) ? [] : [ 'O100ne' ];

        return apply_filters( 'o100_required_migration_names', $needed_migrations );
    }
