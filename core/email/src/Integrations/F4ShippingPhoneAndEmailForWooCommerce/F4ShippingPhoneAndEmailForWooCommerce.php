...
namespace Order100\Notification\Engine\Integrations\F4ShippingPhoneAndEmailForWooCommerce;
...
class F4ShippingPhoneAndEmailForWooCommerce {
    ...
    private function initialize_hooks() {
        add_filter( 'o100_shipping_address_content', [ $this, 'add_shipping_phone_and_email_to_shipping_address' ], 10, 2 );
    }
    ...
    public function add_shipping_phone_and_email_to_shipping_address( $shipping_address, $order ) {
        ...
        if ( ! empty( $shipping_phone ) ) {
            if ( strpos( $shipping_address, '<a href="tel:' . esc_attr( $shipping_phone ) . '">' ) === false ) {
                $html_shipping_phone = '<a class="o100ne-shipping-phone" href="tel:' . esc_attr( $shipping_phone ) . '">' . esc_html( $shipping_phone ) . '</a>';
        ...
        if ( ! empty( $shipping_email ) ) {
            if ( strpos( $shipping_address, '<a href="mailto:' . esc_attr( $shipping_email ) . '">' ) === false ) {
                $html_shipping_email = '<a class="o100ne-shipping-email" href="mailto:' . esc_attr( $shipping_email ) . '">' . esc_html( $shipping_email ) . '</a>';
