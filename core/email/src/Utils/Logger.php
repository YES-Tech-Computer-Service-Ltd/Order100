...
class Logger {
    ...
    public function __construct( $log_directory = null, $max_log_files = 30 ) {
        if ( ! $log_directory ) {
            $log_directory = WP_CONTENT_DIR . '/o100ne-logs';
        }
     ...
     * Message will display in folder wp-content/o100ne-logs
     */
    public function log_exception_message( $ex, $log_type = 'error', $additional_data = null ) {
        $prefix = ( $log_type === 'warning' ) ? __( 'WARNING:', 'order100' ) : __( 'SYSTEM ERROR:', 'order100' );
