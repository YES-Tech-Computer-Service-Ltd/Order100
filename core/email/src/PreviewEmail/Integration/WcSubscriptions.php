...
namespace Order100\Notification\Engine\PreviewEmail\Integration;
...
class WcSubscriptions {
    ...
    private function __construct() {
        add_filter( 'o100_preview_email_woo_additional_order_id', [ $this, 'check_email_subscription' ], 10, 3 );
        add_action( 'o100_preview_email_woo_additional_order_trigger', [ $this, 'trigger_email' ], 10, 3 );
    }
    ...
    public function check_email_subscription( $result, $email_class, $order_id ) {
        $error_text = sprintf( __( 'This is not a valid subscription order. Please select a valid subscription order ID. <br><br>Go to orders page to check %s.', 'order100' ), '<a target="_blank" href="' . admin_url( 'edit.php?post_type=shop_order' ) . '">Order ID</a>' );
