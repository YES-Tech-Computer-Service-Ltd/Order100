...
class ElementsLoader {
    ...
    private function __construct() {
        $dir = new \DirectoryIterator( O100NE_PLUGIN_PATH . '/src/Elements' );
        foreach ( $dir as $fileinfo ) {
            if ( ! $fileinfo->isDot() ) {
                $file_name  = $fileinfo->getFilename();
                $class_name = basename( $file_name, '.php' );
                $class      = 'Order100\Notification\Engine\\Elements\\' . $class_name;
        ...
        do_action( 'o100_register_elements', $this );
    ...
    public static function load_elements( $elements ) {
        ...
        foreach ( $elements as $element ) {
            if ( isset( $element['integration'] ) && '3rd' === $element['integration'] ) {
                $class = 'Order100\Notification\Engine\Integrations\\' . $element['type'];
                ...
                $class = 'Order100\Notification\Engine\Elements\\' . $element['type'];
            }
            if ( ! class_exists( $class ) ) {
                continue;
            }
    ...
    public static function render_elements( $elements, $args ) {
        ...
        $args = apply_filters( 'o100_template_rendering_args', $args, $template_name, $elements );
        ...
        foreach ( $elements as $element ) {
            ...
            if ( ! apply_filters( 'o100_validate_element_before_sending', true, $element, $args ) ) {
                continue;
            }
            ...
            $element_instance = o100ne_get_element( $element['type'] );
            ...
            if ( $is_horizontal ) {
                ...
                ?>
                <td style="padding: 0; width: <?php echo esc_attr( $width ); ?>%; max-width: <?php echo esc_attr( $width ); ?>%;padding-left: 0;padding-right: 0;"><?php o100ne_kses_post_e( $layout ); ?></td>
                <?php
                continue;
            }
            ?>
            <tr>
                <td style="padding: 0;"><?php o100ne_kses_post_e( $layout ); ?></td>
            </tr>
            <?php
        }//end foreach
    }
