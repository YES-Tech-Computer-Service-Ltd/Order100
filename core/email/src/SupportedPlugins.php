...
use Order100\Notification\Engine\Models\AddonModel;
use Order100\Notification\Engine\Utils\SingletonTrait;
/**
 * O100ne SupportedPlugins
 *
...
class SupportedPlugins {
    ...
    private function get_support_status( string $template_id ): string {
        // If the O100ne template data exists, it means the template is supported and ready to be edited
        if ( ! empty( $this->get_o100ne_template_data( $template_id ) ) ) {
    ...
    public function get_o100ne_template_data( $template_id ) {
        $o100ne_emails = \o100ne_get_emails();
        return current( array_filter( $o100ne_emails, fn( $email ) => $email->get_id() === $template_id ) );
    }
