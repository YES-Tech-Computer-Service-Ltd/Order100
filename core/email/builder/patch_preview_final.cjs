...
// We need to disable the old updatePreviewData since we replaced its logic.
const updatePreviewSearchRegex = /const updatePreviewData = \(\) => {[\s\S]*?editor\.refresh\(\);\n  };/;
const updatePreviewReplace = `const updatePreviewData = () => {\n    // Disabled. Backend API handles preview shortcodes now.\n  };`;
content = content.replace(updatePreviewSearchRegex, updatePreviewReplace);

fs.writeFileSync(file, content);
console.log('Successfully patched final version.');
