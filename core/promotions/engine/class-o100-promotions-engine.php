...
class O100_Promotions_Engine {
	...
	public static function init() {
		require_once O100_PATH . 'core/promotions/emails/class-o100-promotions-emails.php';
		O100_Promotions_Emails::init();
		...
		add_filter( 'woocommerce_get_cart_item_from_session', [ __CLASS__, 'strip_promo_metadata_from_session' ], 10, 2 );

	}

	public static function strip_promo_metadata_from_session( $cart_item, $values ) {
		if ( isset( $cart_item['o100_promo_discount'] ) ) {
			unset( $cart_item['o100_promo_discount'] );
		}
		if ( isset( $cart_item['o100_promo_badge'] ) ) {
			unset( $cart_item['o100_promo_badge'] );
		}
		return $cart_item;
	}
	...
	public static function virtual_coupon_data( $data, $code ) {
		...
		foreach ( $promos as $promo ) {
			if ( $promo['source'] === 'loyalty' ) continue; // Skip templates
	...
	public static function get_common_rules_error( $promo, $cart ) {
		...
		if ( $branch !== 'all' ) {
			$current_branch = WC()->session->get( 'o100_location_id', '' );
			if ( $current_branch && $branch !== $current_branch ) {
				return __( 'This promotion is not available at your selected location.', 'order100' );
			}
		}

		// CRM Lists & Tags
		$has_lists = !empty($config['crm_lists']) && is_array($config['crm_lists']);
		$has_tags  = !empty($config['crm_tags']) && is_array($config['crm_tags']);

		if ( $has_lists || $has_tags ) {
			$email = '';
			...
			if ( empty( $email ) ) {
				return __( 'This promotion is exclusive to specific customer groups.', 'order100' );
			}

			if ( ! class_exists( 'O100_Customers_DB' ) ) return __( 'CRM module is missing.', 'order100' );
			...
			if ( $has_lists ) {
				$user_lists = [];
				$db_lists = O100_Customers_DB::get_customer_lists( $cid );
				foreach ( $db_lists as $l ) $user_lists[] = strtolower( trim( $l->slug ) );

				$req_lists = array_map('strtolower', array_map('trim', $config['crm_lists']));
				$logic = isset($config['crm_lists_logic']) ? $config['crm_lists_logic'] : 'in';
				$intersect = array_intersect( $req_lists, $user_lists );

				if ( $logic === 'in' && count( $intersect ) === 0 ) {
					return __( 'This promotion is exclusive to specific customer lists.', 'order100' );
				}
				if ( $logic === 'not_in' && count( $intersect ) > 0 ) {
					return __( 'This promotion is exclusive to specific customer lists.', 'order100' );
				}
			}
			...
			if ( $has_tags ) {
				$user_tags = [];
				$db_tags = O100_Customers_DB::get_customer_tags( $cid );
				foreach ( $db_tags as $t ) $user_tags[] = strtolower( trim( $t->slug ) );

				$req_tags = array_map('strtolower', array_map('trim', $config['crm_tags']));
				$logic = isset($config['crm_tags_logic']) ? $config['crm_tags_logic'] : 'in';
				$intersect = array_intersect( $req_tags, $user_tags );

				if ( $logic === 'in' && count( $intersect ) === 0 ) {
					return __( 'This promotion is exclusive to specific customer tags.', 'order100' );
				}
				if ( $logic === 'not_in' && count( $intersect ) > 0 ) {
					return __( 'This promotion is exclusive to specific customer tags.', 'order100' );
				}
	...
	private static function evaluate_single_condition( $cond, $cart ) {
		...
		switch ( $type ) {
			...
			case 'customer_tag':
				...
				global $wpdb;
				$tbl = O100_Customers_DB::get_table_customers();
				$cid = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$tbl} WHERE wp_user_id = %d", get_current_user_id() ) );
				if (!$cid) return false;
			...
			case 'first_order':
				...
				if ( class_exists( 'O100_Customers_DB' ) ) {
					global $wpdb;
					$tbl = O100_Customers_DB::get_table_customers();
					$customer_data = $wpdb->get_row( $wpdb->prepare( "SELECT total_orders FROM {$tbl} WHERE wp_user_id = %d", get_current_user_id() ) );
					if ( $customer_data ) $order_count = intval( $customer_data->total_orders );
				} else {
					$orders = wc_get_orders([ 'customer_id' => get_current_user_id(), 'limit' => 1, 'return' => 'ids' ]);
					$order_count = count( $orders );
				}
				if ($op === 'yes') return $order_count === 0;
				if ($op === 'no') return $order_count > 0;
				return false;
			...
			case 'prev_orders_count':
				...
				return self::compare_values( $order_count, $op, intval( $val ) );
			case 'total_spent':
				if (!is_user_logged_in()) return false;
				$spent = 0.0;
				if ( class_exists( 'O100_Customers_DB' ) ) {
					global $wpdb;
					$tbl = O100_Customers_DB::get_table_customers();
					$customer_data = $wpdb->get_row( $wpdb->prepare( "SELECT total_spent FROM {$tbl} WHERE wp_user_id = %d", get_current_user_id() ) );
					if ( $customer_data ) $spent = floatval( $customer_data->total_spent );
				} else {
			...
			case 'location_branch':
				$current_branch = WC()->session->get( 'o100_location_id', 'all' );
				$targets = array_map('trim', explode(',', $val));
			...
			case 'delivery_distance':
				$dist = WC()->session->get( '_user_distance', 0 );
	...
	private static function calc_simple( $config, $eligible_items, $cart ) {
		...
		} elseif ( in_array($type, ['fixed', 'fixed_cart', 'fixed_product']) ) {
