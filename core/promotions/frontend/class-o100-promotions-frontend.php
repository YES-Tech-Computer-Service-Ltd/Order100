...
class O100_Promotions_Frontend {
	...
	public static function init() {
		...
		add_filter( 'woocommerce_update_order_review_fragments', [ __CLASS__, 'update_savings_dashboard_fragment' ] );
	...
	public static function get_active_deals() {
		...
		foreach ( $promos as $promo ) {
			// Skip personal coupons generated by loyalty modules or any non-manual sources
			if ( ! empty( $promo['source'] ) && $promo['source'] !== 'manual' ) {
				continue;
			}
	...
	public static function update_savings_dashboard_fragment( $fragments ) {
		ob_start();
		self::render_savings_dashboard();
		$html = ob_get_clean();
		$fragments['.o100-savings-dashboard-wrapper'] = $html;
		return $fragments;
	}

	public static function render_savings_dashboard() {
		echo '<div class="o100-savings-dashboard-wrapper">';
		...
		if ( class_exists( 'O100_Shipping' ) ) {
			$shipping_module = O100_Shipping::instance();
		...
		if ( class_exists( 'O100_Loyalty_Engine' ) && class_exists( 'O100_Loyalty_DB' ) ) {
			...
			if ( $earned_points > 0 ) {
				...
				if ( ( $settings['checkout_earn_message_enable'] ?? 'yes' ) === 'yes' ) {
					...
					function o100ApplyCheckoutReward(btn, code, productId) {
						...
						jQuery.ajax({
							...
							error: function(err) {
								...
								if (typeof o100ShowToast === "function") o100ShowToast(msg, "error");
	...
	public static function render_popup() {
		...
		global $post;
		if ( $popup_location === 'shop' && is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, 'o100_menu' ) || has_shortcode( $post->post_content, 'order100_food_menu' ) ) ) {
			$show = true;
		}
			...
			$(document).ready(function() {
				...
				function applyToCards() {
					$('.o100-product-card, .fditem-list, .item-grid').each(function() {
						...
						if (!promo) {
							$card.data('o100-promo-injected', true); // Mark as checked to skip future scans
							return;
						}
				...
				applyToCards();
				// Only re-scan when new product cards appear (menu navigation, not every AJAX)
				$(document.body).on('o100_menu_updated wc_fragments_loaded', function() {
					setTimeout(applyToCards, 300);
				});
